...
export interface AnalyticsEventData {
  eventType:
    'page_view' | 'user_action' | 'campaign_view' | 'donation_completed';
  userId?: string;
  sessionId: string;
  timestamp: Date;
...
export class AnalyticsProcessor {
  ...
  async handlePageView(job: Job<AnalyticsEventData>) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to track page view for session ${job.data.sessionId}`,
        error,
      );
      throw error;
    }
  ...
  async handleUserAction(job: Job<AnalyticsEventData>) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to track user action for user ${job.data.userId}`,
        error,
      );
      throw error;
    }
  ...
  async handleCampaignView(job: Job<AnalyticsEventData>) {
    ...
    try {
      ...
      this.logger.log(
        `Campaign view tracked for session ${job.data.sessionId}`,
      );

      return { success: true, tracked: true };
    } catch (error) {
      this.logger.error(
        `Failed to track campaign view for session ${job.data.sessionId}`,
        error,
      );
      throw error;
    }
  ...
  async handleDonationCompleted(job: Job<AnalyticsEventData>) {
    ...
    try {
      ...
      this.logger.log(
        `Donation completion tracked for user ${job.data.userId}`,
      );

      return { success: true, tracked: true };
    } catch (error) {
      this.logger.error(
        `Failed to track donation completion for user ${job.data.userId}`,
        error,
      );
      throw error;
    }
